...
const AdminDashboard = () => {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/admin/doctors`, {
  ...
  const handleApprove = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/admin/approve-doctor/${id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const [statsRes, appointmentsRes, specializationsRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/admin/analytics/stats`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/admin/analytics/appointments-over-time`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/admin/analytics/doctor-specializations`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const fetchAllFeedbacks = async () => {
    try {
      const token = localStorage.getItem("adminToken");
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/admin/analytics/all-feedbacks`, {
        headers: { Authorization: `Bearer ${token}` },
      });
