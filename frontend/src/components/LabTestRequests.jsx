...
const LabTestRequests = () => {
  ...
  const fetchTests = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return; // prevent using data as an array if it's an error
      }
  
      // Only log if it's actually an array
      if (Array.isArray(data)) {
        data.forEach(t => console.log(t.testName, t.status, t.result));
      }
  
      setTests(data); // Only set if fetch succeeded
  ...
  const handleUpdate = async (id, status) => {
    try {
      ...
      else { toast.success("Status updated"); fetchTests(); }
...
const handleUpload = async () => {
  ...
  const handleView = async (testId) => {
    ...
    try {
      ...
      setViewPdfUrl(blobUrl); // show modal
              ...
              {tests.map((test) => (
                <tr key={test._id} className="border-b">
                  ...
                  <td className="p-2 space-x-2">
                    {test.status === "pending" && (
                      <>
                        <button onClick={() => handleUpdate(test._id, "accepted")} className="bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600">Accept</button>
                        <button onClick={() => handleUpdate(test._id, "rejected")} className="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">Reject</button>
                      </>
                    )}
      ...
      {selectedTest && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-5 rounded shadow-lg w-96">
            ...
            <input type="file" onChange={(e) => setFile(e.target.files[0])} className="mb-3 w-full"/>
            <div className="flex justify-end space-x-2">
              <button onClick={() => setSelectedTest(null)} className="px-3 py-1 rounded border">Cancel</button>
              <button onClick={handleUpload} className="px-3 py-1 rounded bg-blue-500 text-white hover:bg-blue-600">Upload</button>
            </div>
          </div>
        </div>
      )}
